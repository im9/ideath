...
| `partialgain <idx> <value>` | Modal per-partial output gain (Rings-style Position knob). `idx` 0-15 targets a single partial; `value` 0-4 (default 1.0). `value=0` mutes that partial audibly but preserves its BP state so it can be reintroduced without a fresh strike. Shape which modes ring loudest without changing pitch or decay. e.g. `modal 220 8 0.7 0.2`, then `partialgain 0 0` `partialgain 3 2` to emphasise the 4th partial. |
