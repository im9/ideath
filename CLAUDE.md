...
- **ModalResonator** — Bell engine: N parallel BPs (modes) with per-partial Q derived from decay (`Q = π × fc × decay / ln(1000)`), struck by a short noise burst; piano-string inharmonicity stretch (`f_i = fundamental × ratio_i × sqrt(1 + B × ratio_i²)`); Nyquist-muted partials; per-partial output gain (`setPartialGain`, default 1.0, clamped `[0, 4]`) for Rings-style Position-knob amplitude shaping — `gain=0` mutes audibly but preserves BP state for reintroduction
...
- **Output levels** — most primitives output ±1.0 for ±1.0 input, but resonant filters and reverbs can exceed this significantly. The plugin layer MUST place a PeakLimiter after the signal chain. Per-primitive output ceilings for ±1.0 input:
  ...
  | ModalResonator | ±N × max(gain) (up to ±64) | N parallel BPs with per-partial Q derived from decay; BP output multiplied by Q to compensate 0 dB-peak normalisation → per-partial peak ≈ 1, then × per-partial gain (default 1, `[0, 4]`) → N partials sum to ≈ ±N × max(gain) in worst-case phase alignment (paper worst case only — typical strike output stays well below because per-partial impulse responses don't peak simultaneously and Position-style weightings redistribute rather than uniformly amplify) |
...
7. **Wire the primitive into the REPL** — required for any primitive that produces or transforms audio (the "REPL exposure guideline" above is not optional, it's part of the definition of "done"). For a source: add to `SourceType` in `tools/repl/SharedState.h`, add `VoiceParams` fields for its setters, add a member + `prepare()` call in `tools/repl/AudioEngine.{h,cpp}`, switch case in the source-dispatch in `AudioEngine::process()`, command parser entry in `tools/repl/CommandParser.cpp`, help-text line, and `tools/repl/COMMANDS.md` row. For an effect: same minus the source dispatch. Primitives that only bundle other primitives (Voice, Polyphony) are exempt because the REPL itself is that role. Skipping this step means downstream plugin work can't audition the primitive against the reference engine.
8. **Run `/audit` (the audit skill)** — required for any new primitive or non-trivial change to an existing one.  History has shown narrow per-file review misses entire bug classes (canonical-reference omissions, reversed signs that match a correct comment, switch/case copy-paste, fixes that were applied to the REPL but not the library).  The audit skill at `.claude/skills/audit.md` is structured around the bug families that have actually shown up here.
9. **Run `make bench`** — required for any new primitive or change that touches a hot loop / `process()` body.  The bench suite at `benchmarks/bench_primitives.cpp` exists to catch performance regressions before they reach the downstream plugin / iOS targets where headroom is tight.

Steps 7–9 are not optional polish — they are part of the definition of "done" for primitive work.  Skipping them is how we got the bugs that prompted this checklist.  When in doubt about whether a change qualifies as "non-trivial", run them anyway; they're cheap compared to debugging a regression in the plugin layer.
