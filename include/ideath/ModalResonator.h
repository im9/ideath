...
/// its own Q at the sum stage, so each partial's peak amplitude is bounded
/// by ≈ excitation_amplitude (≤1 from the noise burst).  N partials sum in
/// worst-case phase alignment, so the worst-case output is ≈ ±N (= ±16 at
/// kMaxPartials=16).  Typical strike output is well under ±N because the
/// per-partial impulse responses do not peak simultaneously after the
/// stochastic 2 ms noise excitation.  The plugin layer's PeakLimiter is
/// responsible for the final ceiling.
///
